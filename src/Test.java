public class Test {
    ...
    static int Test7(){
        ...
        if (node.left.key != 3 || node.left.height != 0) return 0; //line 290 modified
        if (node.right.key != 10 || node.right.height != 1) return 0;
        if (node.right.left.key != 5 || node.right.left.height != 0) return 0; //line 292 modified
    ...
    static int Test9(){
        ...
        /* Test min & max */
        node = avl.Min();
        if (node.key != 4) return 0;

        node = avl.Max();
        if (node.key != 62) return 0;
